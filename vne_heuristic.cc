...
int main(int argc, char* argv[]) {
  ...
  auto node_map = CreateInitialNodeMap(physical_topology.get(),
  ...
  // for (int i = 0; i < (*partitions)[PRIMARY].size(); ++i)
  //   printf(" %d", (*partitions)[PRIMARY][i]);
  // printf("\n");
  // for (int i = 0; i < (*partitions)[BACKUP].size(); ++i)
  //   printf(" %d", (*partitions)[BACKUP][i]);
  // printf("\n");
  auto& primary_partition = (*partitions)[PRIMARY];
  auto& backup_partition = (*partitions)[BACKUP];

  auto emap = EmbedVN(physical_topology.get(), virt_topology.get(),
                      primary_partition, primary_node_map);
  auto semap = EmbedVN(physical_topology.get(), virt_topology.get(),
